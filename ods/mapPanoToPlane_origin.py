...
if __name__ == '__main__':
    ...
    parser.add_argument('--phi', type=float, default=(math.pi/3), help='middlepoint phi of the view in the pano') # in [0, 2*math.pi]
    parser.add_argument('--theta', type=float, default=(math.pi/2.0), help='middlepoint theta of the view in the panoFN2') # in [0, math.pi]
    ...
    plane_x_direction = np.cross(direction, up)
    ...
    # convert the direction vector of each pixel into spherical coordinates
    length = np.sqrt(np.sum(np.stack((np.power(direction_x, 2), np.power(direction_y, 2), np.power(direction_z, 2))), axis=0))
    direction_theta = np.arccos(direction_y / length)
    direction_phi = np.arctan2(-direction_z, direction_x) + math.pi

    # convert the spherical coordinates into pixel lookups in the pano
